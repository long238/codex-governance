...
适配后，根 `AGENTS.md` 维护过程与边界，`docs/README.md` 按“具体功能 → 主章节”和“专项影响 → 追加阅读”组织入口。配置、日志、依赖和交付事实优先归入目标项目已有文档，不为凑齐分类创建空专项文件；具体字段、命令和策略必须从目标项目核对。

治理规则同时约束后续维护：检查业务与配置的直接和间接影响，更新已有主章节及导航；配置变更验证实际生效，守卫规则变更验证能识别违规样例，结果区分通过、失败、跳过与未执行。已批准任务中有 WORKLOG 记录的同步迁移中间态不重复触发审批，完成前仍须消除差异；Decision 和 Playbook 默认只记录当前任务的新决定和已验证经验。
...
- 文档更新同步维护功能入口、章节锚点和唯一事实归属；完整测试或交付清单以可执行来源为准，文档解释用途和覆盖边界。
